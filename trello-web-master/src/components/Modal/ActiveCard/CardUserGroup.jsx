...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMemberIds }) {
  /**
   ...
   * Vì thế dựa vào mảng board.FE_allUsers và card.memberIds rồi chúng ta tạo ra một mảng
   * FE_CardMembers chứa đủ thông tin của User để hiển thị ra ngoài giao diện, bởi mặc định trong
   * card chỉ lưu mỗi Id của User thôi (memberIds)
   */
        ...
        <Box sx={{ p: 2, maxWidth: '260px', display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
          {board.FE_allUsers.map((user, index) => (
            <Tooltip title={user.displayName} key={index}>
              ...
              <Badge sx={{ cursor: 'pointer' }} overlap="rectangular" anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} badgeContent={cardMemberIds.includes(user._id) ? <CheckCircleIcon fontSize="small" sx={{ color: '#27ae60' }} /> : null} onClick={() => handleUpdateCardMembers(user)}>
