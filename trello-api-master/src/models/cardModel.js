...
const createNew = async (data) => {
  try {
    ...
    // Biến đổi data liên quan tới ObjectId chuẩn chỉnh
    const newCardToAdd = {
      ...validData,
...
/**
 * - Trong JS, ngược lại với push (thêm phần tử vào cuối mảng) sẽ là unshift (thêm
 *   phần tử vào đầu mảng)
 ...
 *   thêm phần tử vào đầu mảng trong MongoDB.
...
const unshiftNewComment = async (cardId, commentData) => {
  try {
    const result = await GET_DB()
      .collection(CARD_COLLECTION_NAME)
      .findOneAndUpdate({ _id: new ObjectId(cardId) }, { $push: { comments: { $each: [commentData], $position: 0 } } }, { returnDocument: 'after' })
...
const updateMembers = async (cardId, incomingMemberInfor) => {
  try {
    let updateCondition = {}
    if (incomingMemberInfor.action === CARD_MEMBER_ACTIONS.ADD) {
      updateCondition = { $push: { memberIds: new ObjectId(incomingMemberInfor.userId) } }
    }
    if (incomingMemberInfor.action === CARD_MEMBER_ACTIONS.REMOVE) {
